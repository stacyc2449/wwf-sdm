...
def layer_path(period_dir, feature_name):
    # ---- EDIT THIS STRING PATTERN TO MATCH YOUR DATA ----
    return rf"chelsa_clim\CHELSA_{feature_name}_{period_dir}_V.2.1.tif"
    # ------------------------------------------------------
...
for feature_name in training_feature_names:
    in_path = layer_path(period_dir, feature_name)
    ...
    with rasterio.open(in_path) as src:
        ...
        out_path = os.path.join(out_dir, f"current_{feature_name}.tif")
...
# Apply mask_mid to both X and y
    ...
    with rasterio.open("outputs/" + command[3] + "/current_" + command[3] + "_ensemble.tif", "w", **profile) as dst:
